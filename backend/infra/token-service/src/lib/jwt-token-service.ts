...
const { JWT_ACCESS_TOKEN_HASH_KEY, JWT_REFRESH_TOKEN_HASH_KEY } = envsSchema.parse(process.env);
...
export class JWTTokenService implements TokenService {
  ...
  verifyRefreshToken(refreshToken: string): Pick<TokenPayload, 'sub' | 'iat' | 'exp'> {
    return verify(refreshToken, JWT_REFRESH_TOKEN_HASH_KEY) as Pick<TokenPayload, 'sub' | 'iat' | 'exp'>;
  }
