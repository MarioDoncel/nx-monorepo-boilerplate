import { MemoryCache, createCache, memoryStore } from 'cache-manager';
...
export class InMemoryDataStoreLocalCacheManager implements InMemoryDataStore {
  ...
  async setHash<K extends never>(
    key: K,
    value: K extends never ? Hashes[K] : never
  ): Promise<void> {
  ...
  async queryHash<K extends keyof Hashes>(
    ...
    const result = (await this.memoryCache.get(key)) as
      | (K extends never ? Hashes[K] : never)
      | undefined;
  ...
  async setString(
    ...
    const ttl = options?.expireInSeconds
      ? options.expireInSeconds * 1000
